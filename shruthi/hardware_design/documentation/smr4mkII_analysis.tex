...
\item LM13700 Darlington buffers. The ``OTA-C-buffer" configuration is actually very common, but I could not get it to work with the LM13700 in a satisfying way. The main problem was self-oscillation, which, for different values of the cutoff frequency, did not appear at the same values of the resonance VCA gain. Either a loud, Curtis-like, distorted sine wave for high cutoff values and a normal sine wave for the lowest values ; or a normal sine wave for high cutoff values and no self-oscillation for the lowest values. It was as if the Darlington buffers had a very slight high-pass effect.
