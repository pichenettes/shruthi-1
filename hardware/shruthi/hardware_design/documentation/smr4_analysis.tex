...
\paragraph{OTA small voltage constraint} The signal output by the digital oscillator is in the $0-5V$ range, then AC-coupled and scaled by a gain of $0.22$ in the input mixer. The signal at the input of the OTA-C cells has thus a range of $\frac{R_s}{R_b + R_s} \times 0.22 \times 5 = 25mV$.
...
\item LED clipping in the feedback loop to tame the resonance strength~-- ideally one shouldn't have to trim the resonance feedback gain.
\end{itemize}
